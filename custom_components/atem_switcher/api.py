...
class AtemSourcesApiClientError(Exception):
    """Exception to indicate a general API error."""


class AtemSourcesApiClientCommunicationError(
    AtemSourcesApiClientError,
):
    """Exception to indicate a communication error."""


class AtemSourcesApiClientAuthenticationError(
    AtemSourcesApiClientError,
):
    """Exception to indicate an authentication error."""


class AtemSourcesApiClient:
    """Sample API Client."""
    ...
    async def wait_for_connection(self) -> None:
        ...
        log.debug("Waiting for connection to ATEM to complete")
        while True:
            if self._connected:
                return
    ...
    def set_source(self, source: str) -> Any:
        ...
        log.debug("Switching to input %s", source)
        me = self._switcher.atem.mixEffects.mixEffect1
        for _, v in self._videoSources._values.items():
            if self._switcher.inputProperties[v.value].longName == source:
                log.debug("Switching to input %s", int(v.value))
                self._switcher.setProgramInputVideoSource(me, int(v.value))
                return
        log.warning("Unable to find input %s", source)
